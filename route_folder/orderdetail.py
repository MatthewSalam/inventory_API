...
async def create_order_detail(order_req: OrderDetailBase, db: dbDepend):
...
async def get_all_order_details(db: dbDepend):
...
async def get_all_inactive_order_details(db: dbDepend):
...
async def get_order_detail(detail_id: int, db: dbDepend):
...
async def update_order_detail(detail_id: int, updated: OrderDetailBase, db: dbDepend):
    """Update order_detail."""
    detail = db.query(Order_Detail).filter(Order_Detail.id == detail_id, Order_Detail.is_active == True).first()
    if not detail:
        raise HTTPException(status_code=404, detail="Order_Detail not found")
...
async def deactivate_order_detail(detail_id: int, db: dbDepend):
...
async def reactivate_order_detail(detail_id: int, db: dbDepend):
